...
def next_move_cut_angle(hunter_position, hunter_heading, target_measurement, max_distance, OTHER = None):
    ...
    xy_estimate_third_step = target_measurement
    ...
    if OTHER is None:
        ...
        if len(coords) == 1:
            ...
            newR.move_in_circle()
            xy_estimate_third_step = newR.x, newR.y

    coords.append(target_measurement)
    OTHER = (distances, angles, coords)

    distance_third_step = distance_between(hunter_position, xy_estimate)
    ...
    elif distance_second_step < max_distance:
        distance = distance_second_step
        xy_estimate = xy_estimate_second_step
    # else head to three steps forward
    else:
        distance = distance_third_step
        xy_estimate = xy_estimate_third_step
